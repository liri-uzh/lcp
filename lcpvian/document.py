...
async def document(request: web.Request) -> web.Response:
    ...
    job = await _document(ctx, schema, corpus, doc_id, user, room, corpus_conf)
    info: dict[str, str] = {
        "status": "started",
...
async def image_annotations(request: web.Request) -> web.Response:
    ...
    job = await _image_annotations(ctx, corpus_conf, layer, ids, xy_box, user, room)
    info: dict[str, str] = {
        "status": "started",
        "job": "" if job is None else job.job_id,
    }
...
async def annotations(request: web.Request) -> web.Response:
    ...
    corpus_conf = request.app["config"][str(corpus)]
    job = await _annotations(
        ctx, corpus_conf, anchor, rang, str(corpus), lang, limit, user, room
    )
    info: dict[str, str] = {
        "status": "started",
        "job": "" if job is None else job.job_id,
...
async def clip_media(request: web.Request) -> web.Response:
    ...
    job = await _clip_media(ctx, corpus_conf, span, doc_id, user, room)
    info: dict[str, str] = {
        "status": "started",
        "job": "" if job is None else job.job_id,
    }
