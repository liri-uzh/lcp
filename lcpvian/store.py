...
async def fetch_queries(request: web.Request) -> web.Response:
    ...
    job: Job | None = await enqueue(
        "store.fetch_queries", user, room, query_type, queue="internal"
    )
    info: dict[str, str] = {
        "status": "started",
...
async def store_query(request: web.Request) -> web.Response:
    ...
    job: Job | None = await enqueue("store.store_query", *args, queue="internal")
    info: dict[str, str] = {
        "status": "started",
        "job": "" if job is None else job.job_id,
        "query_id": str(idx),
...
async def delete_query(request: web.Request) -> web.Response:
    ...
    job: Job | None = await enqueue(
        "store.delete_query", user_id, room_id, query_id, queue="internal"
    )
    info: dict[str, str] = {
        "status": "started",
        "job": "" if job is None else job.job_id,
