WITH RECURSIVE fixed_parts AS
  (SELECT "t"."char_range" AS "t_char_range",
          ...
          extract('year'
                  FROM ("u_agent"."agent"->>'annee_naissance')::date) AS "year_u_agent_annee_naissance"
   ...
   WHERE "t_form"."form_id" = "t"."form_id"
     ...
     AND (("u_agent"."agent"->>'annee_naissance')::text ~ concat('[0-9]', chr(123), '4', chr(125), '-[0-9]', chr(123), '2', chr(125), '-[0-9]', chr(123), '2', chr(125), '')
          AND (extract('year'
                       FROM ("u_agent"."agent"->>'annee_naissance')::date))::numeric > (1970)::numeric) ),
               match_list AS
  (SELECT fixed_parts."t" AS "t",
          ...
          fixed_parts."year_u_agent_annee_naissance" AS "year_u_agent_annee_naissance"
   FROM fixed_parts),
               res1 AS
  ...
  (SELECT 2::int2 AS rstype,
          jsonb_build_array(FALSE, "year_u_agent_annee_naissance", "u_agent_region", frequency)
   FROM
     (SELECT "year_u_agent_annee_naissance",
             "u_agent_region" ,
             count(*) AS frequency
      FROM match_list
      GROUP BY "year_u_agent_annee_naissance",
               "u_agent_region") x) ,
               res0 AS
