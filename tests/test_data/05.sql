WITH RECURSIVE fixed_parts AS
  (SELECT "interruptee"."char_range" AS "interruptee_char_range",
          ...
          "interruptee_agent"."agent"->>'region' AS "interruptee_agent",
          ...
          "interruptor_agent"."agent"->>'region' AS "interruptor_agent",
  ...
  (SELECT 2::int2 AS rstype,
          jsonb_build_array(interruptee_agent, interruptor_agent, frequency)
   FROM
     (SELECT interruptee_agent,
             interruptor_agent ,
             count(*) AS frequency
      FROM match_list
      GROUP BY interruptee_agent,
               interruptor_agent) x) ,
               res0 AS
